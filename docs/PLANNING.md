...
| 2026-06-05 | D1: added `Player.aiMemory`, `Player.intelLedger`, `Player.researchAccumulated`, `Player.shipStatBonuses`, `Player.productionBonuses`, `Player.specialBonuses`, `Player.taxRate`, `Player.unlockedBuildings/Hulls/Weapons/Specials` fields, and `Building.level` field; removed `score` from `Player` (lives on `GameState`); Ship semantics clarified as per-individual with stack grouping on `Fleet.ships` (per stack: `{designId, count, hpCurrent, hpMax, experience}`); `Spy` and `Mission` records consolidated in D1.2 with D12.1 only defining `MissionKind` ADT and assignment logic | REVIEW-NOTES 8.1, 8.2, 8.3, 8.4, 8.5, 8.9, 8.10, 10.4, 10.5, 10.10 — multiple sections referenced fields/types that didn't exist on `Player`/`Building`/`Spy`/`Mission` |
| 2026-06-05 | D1: added cross-entity helpers (`totalFleetStrength`, `homeworldBonus`, `playerTotalPlanets`, `habitablePlanetIds`); added `WeightVector` and `PlayerIntel` types; added `MISSION_DURATION` constants table; added `TURN_WARNING_THRESHOLD = 10`, `TAX_RATE_MIN/MAX/DEFAULT`, `BUILDING_LEVEL_MULTIPLIER(level) = level` to game-wide constants | REVIEW-NOTES 8.6, 8.7, 8.8, 10.1, 10.8 — helpers and constants referenced but not declared |
| 2026-06-05 | D1: `WeaponKind.Fighter`/`Bomber` renamed to `FighterBay`/`BomberBay` to avoid colliding with `Hull.Fighter`/`Hull.Bomber` | REVIEW-NOTES 8.12 — D7.1 (hulls) and D7.2 (weapons) shared the names `Fighter`/`Bomber` |
| 2026-06-05 | D1: `Event` vs `CombatEvent` vs `FleetEvent` ownership boundaries documented; `CombatEvent` and `FleetEvent` are pipeline types (not stored in `state.events`); the corresponding `Arrived`/`NoEncounter`/`Encounter` `Event` kinds are what D8.4 appends to the canonical log | REVIEW-NOTES 6.5, 8.11, 9.9 — three near-duplicate event-like types had unclear ownership |
| 2026-06-05 | D1: `StarvationEvent { planet, lostPopulation }` renamed to `populationLost` to match D10.4's `PillageEvent { planet, populationLost, ... }` payload naming convention | REVIEW-NOTES 10.2 — inconsistent payload naming |
| 2026-06-05 | D1: D8.4's `FleetEvent.Encounter.sideA/sideB` renamed to `sideAId/sideBId` to match D1.2's `CombatEvent` naming convention | REVIEW-NOTES 10.3 — inconsistent event field suffix |
| 2026-06-05 | D1: added `PlanetColonized` and `SpyProduced` event kinds to the event-kind index | REVIEW-NOTES — these were emitted by D8.6 (new) and D12.1 (existing) but never added to the canonical table |
| 2026-06-05 | D3: `HyperExpansion` removed from `Trait` catalog (no race assigned); `ColonizationSpeed`/`MaxColonies` removed from `Modifier` ADT (v1 has no consumer — colonization is one-shot D8.6); `Tolerant` trait added with `Diplomacy(1)` modifier; `Subterranean` `traitModifiers` now `{Morale(1), GroundDefense(1)}` and "no surrender" is data-driven via `hasTrait(state, defender, Subterranean)` in D10.3; `SpyDetection` semantic explicitly documented (applied to *defending* player — the trait's natural reading was inverted; v1 keeps the inverted semantic for MoO compatibility) | REVIEW-NOTES 8.13, 8.14, 8.15, 9.5, 10.6 — D3.2 trait table gaps, dead code, ambiguous modifier semantics |
| 2026-06-05 | D5: orchestrator reordered so `D5.6 (morale)` runs before `D5.3 (industry)` so industry can scale by `morale/100` without silently using last turn's morale | REVIEW-NOTES 7.1 — critical sequencing bug |
| 2026-06-05 | D5: `BuildBuilding(kind)` cost formula pinned as `state.buildings[kind].baseCost × BUILDING_LEVEL_MULTIPLIER(state.buildings[kind].level)`; `Player.totalProduction` dropped from D14/P12 dependency list (production is not in the v1 score formula — flows through `state.score` after D4 recompute if added in v2) | REVIEW-NOTES 6.7, 8.3, 8.9, 10.7 — undefined helper, missing level field, dangling dependency |
| 2026-06-05 | D5: `taxRate` slider range pinned via D1.1's `TAX_RATE_MIN/MAX/DEFAULT` constants; D5.5 dependency table updated | REVIEW-NOTES 8.7 |
| 2026-06-05 | D6: `Player.researchAccumulated` field added to D1.2 (D6.3 already referenced it) | REVIEW-NOTES 10.4 |
| 2026-06-05 | D8: new `D8.6 Colonization` chunk added; `PlanetColonizedEvent` added to D1 event-kind index; colonization requirement is a fleet with `Hull.Colony Ship` (or `Special.ColonyModule`) arriving at an uninhabited planet | REVIEW-NOTES 10.12 — colonization loop not defined; only homeworld assignment (D3.5) was documented |
| 2026-06-05 | D8: `Fleet.ships` clarified as a `List<{designId, count, hpCurrent, hpMax, experience}>` stack grouping with HP tracked per-stack; per-individual ships are v2 | REVIEW-NOTES 8.10 — D8.5 used stack semantics but D1.2's `Ship` record had both `count` and `hp`, ambiguous |
| 2026-06-05 | D9: `CombatStats.speed` documented as cosmetic in v1 (feeds fleet speed display only); does not affect initiative (computer stat does), movement (D8.2 uses `Hull.baseWarpRange`), or combat outcomes | REVIEW-NOTES 9.8 |
| 2026-06-05 | DECOMPOSITION.md: D8 chunks updated to include D8.6; D9 chunks updated to include D9.7 (boarding) as an explicit v2 placeholder | REVIEW-NOTES 9.4 — D9.7 was referenced inside D9 but absent from DECOMPOSITION.md |
| 2026-06-05 | D11: `distancePenalty(fromPlanet, toPlanet)` pinned as `max(0.5, 1.0 - 0.01 × euclideanDistance(parentStar(fromPlanet), parentStar(toPlanet)))` — distance is between the planets' *parent stars* (planets don't carry coordinates) | REVIEW-NOTES 9.7 — distance function was undefined |
| 2026-06-05 | D11: research-agreement split phrasing reconciled — "unspent research" (D11.2) and "after tech-acquisition deduction" (D5.5) are the same operation; D5.5 is the precise phrasing | REVIEW-NOTES 10.11 |
| 2026-06-05 | D12: `Spy` and `Mission` record shapes consolidated in D1.2 (D12.1 now only defines `MissionKind` ADT and assignment logic); `duration(missionKind)` references D1.1's `MISSION_DURATION` table; detection formula wired to apply the defending player's `SpyDetection` trait modifier | REVIEW-NOTES 8.4, 8.5, 9.5, 10.8 — duplicate record definitions, duration values scattered, `SpyDetection` had no consumer |
| 2026-06-05 | D13: `aiDefaultStrategy(race)` priority list documented as **first-match wins** (resolves both empty-hints and contradictory-hints cases; contradictory races default to highest-priority matching strategy); `AIPipelineEvent` documented as a debug/per-observability event consumed by P13 (AI Inspector) — not by gameplay logic | REVIEW-NOTES 9.6, 10.6 |
